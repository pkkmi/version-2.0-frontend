...
def api_register():
    ...
    try:
        ...
        try:
            ...
            # If there's an error checking, assume user doesn't exist and continue
        ...
        current_app.logger.error(f"Unexpected error in api_register: {e}")
...
def api_login():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error(f"Error during login process: {e}")
            return jsonify({"error": "Login failed due to database error"}), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error in api_login: {e}")
...
def api_get_user():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error in api_get_user: {e}")
...
def api_update_user():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error in api_update_user: {e}")
...
def api_consume_words():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error(f"Error consuming words: {e}")
            return jsonify({"error": "Failed to consume words due to database error"}), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error in api_consume_words: {e}")
        return jsonify({"error": "Failed to consume words due to server error"}), 500
